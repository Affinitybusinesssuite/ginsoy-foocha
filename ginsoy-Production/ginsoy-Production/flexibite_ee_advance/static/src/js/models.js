odoo.define('flexibite_ee_advance.models', function (require) {
    ...
    models.load_fields("pos.order", ['is_delivery'], ['picking_id'], ['x_studio_srb_inv']);
	//models.load_fields("stock.picking",['picking_id']);
...
//    	},
	...
	set_order_x_studio_number: function(order_x_studio_number) {
		console.log('Hello2');
        this.order_x_studio_number = order_x_studio_number;
        },
        get_order_x_studio_number: function() {
		console.log('Hello1');
            return Number(orderNo++);//this.order_x_studio_number;
        },
        generate_unique_id: function() {	 
          
	//var last_id = self.env['table.name'].search([], order='id desc')[0].id
		//var orderlength =this.orders.length;
...
//    	},
	...
	set_srb_inv: function(x_studio_srb_inv){
            this.set('x_studio_srb_inv', x_studio_srb_inv);
        },
	get_srb_inv: function(x_studio_srb_inv){
            return this.get('x_studio_srb_inv');
        },	
     ...
     //Rounding
		...
		getSubTotal: function() {
			var subtotal = this.get_total_with_tax();
        	if(this.get_rounding_status()){
	        	if(this.pos.config.enable_rounding && this.pos.config.rounding_options == 'digits'){
	        		var value = round_pr(total);
	                return value;
	        	}else if(this.pos.config.enable_rounding && this.pos.config.rounding_options == 'points'){
	        		var subtotal = this.get_total_without_tax() - this.get_total_tax();
	                var value = decimalAdjust(subtotal);
	                return value;
	        	}
        	}else {
        		return subtotal
