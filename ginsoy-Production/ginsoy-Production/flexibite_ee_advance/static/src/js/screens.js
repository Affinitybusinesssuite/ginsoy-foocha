	...
	var _t = core._t;	    
		...
		loaded: function (selfr, employeer) {
			...
			selfr.employee_name_by_idr[employeer[i].id] = employeer[i];
			   $('#rider_selection').append($('<option>', {
	    			 value: 1,
   				 text: 'My option'
				}));
		            }		    
			//$('#rider_selection').append($("<option>" + selfr.employee_name_by_idr[employeer].name + "</option>").attr("value", selfr.employee_name_by_idr[employeer].name).attr("id", selfr.employee_name_by_idr[employeer].id))		    }
		    /*console.log("RIDERS");
		    for (var employeer in selfr.employee_name_by_idr) {
                	$('#rider_selection').append($("<option>" + this.pos.employee_name_by_idr[employeer].name + "</option>").attr("value", this.pos.employee_name_by_idr[employeer].name).attr("id", this.pos.employee_name_by_idr[employeer].id))
            	    } */
		}
	    });
...
//            };
		...
		console.log("TAXES");
    		...
    		var total = order ? order.get_total_with_tax() : 0;
    		var totalwithoutTax = order ? order.get_total_without_tax() : 0;
    		var subtotal = total - totalwithoutTax;
                $(".taxes").html(totalwithoutTax);
    		...
    		if(!order.get_is_delivery()){
			 var deliver_product_id = self.pos.config.delivery_product_id[0];
	    		var deliver_amt = self.pos.config.delivery_amount;
	    		 var product = self.pos.db.get_product_by_id(deliver_product_id);
    			var line_deliver_charges = new models.Orderline({}, {pos: self.pos, order:order, product: product});
	        		line_deliver_charges.set_quantity(0);
	        		line_deliver_charges.set_unit_price(deliver_amt || 0);
	        		line_deliver_charges.set_delivery_charges_color(true);
	        		line_deliver_charges.set_delivery_charges_flag(true);
...
//    		}
    		var pos_type = self.pos.config.pos_type;
				...
				{
       	 			$("div#delivery_mode").trigger('click');
				}
       	 			//$("div#drive_through_mode").addClass('selected-menu');
       	 			$("div#dine_in_mode").removeClass('selected-menu');
       	 			$("div#online_mode").removeClass('selected-menu');
...
//            }
        ...
        render_change: function() {
           ...
           // this.$('.taxes').html(this.format_currency(this.pos.get_order().getNetTotalTaxIncluded()));
		console.log((this.format_currency(this.pos.get_order().getNetTotalTaxIncluded())));
        },
        print_xml: function() {
            var order = this.pos.get_order();
            var env = {
...
//	           	 			}
		...
		var employee_id = +$(event.target).parent().parent().parent().find(" .rider_selection :selected").attr('id');
		var employee_name = $(event.target).parent().parent().parent().find(" .rider_selection :selected").text();
			selectedOrder.order_rider = employee_name;
			selectedOrder.employee_id = employee_id;
			var d = new Date();
			var H = d.getHours();
			var M = d.getMinutes();
			console.log(H+":"+M);
		        selectedOrder.set_srb_inv(H+":"+M);
			//selectedOrder.set_picking_id(result.picking_id);
			selectedOrder.set_delivery_type('pending');
		        selectedOrder.customer_email = H+":"+M;
		               	}
	            	}
//	            }
	            selectedOrder.name = result.pos_reference;
		    this.pos.push_order(selectedOrder);
		    return self.pos.db.notification('success',_t('Rider set successfully!'));
...
//            }	
	...
	console.log("RIDER");
		var selected = $('#rider_selection tr');
		console.log(selected);
            for (var employeer in this.pos.employee_name_by_idr) {
                $('.rider_selection').eq(row_index).append($("<option>" + this.pos.employee_name_by_idr[employeer].name + "</option>").attr("value", this.pos.employee_name_by_idr[employeer].name).attr("id", this.pos.employee_name_by_idr[employeer].id))
            }
	    */
        },
		...
		var employees = this.pos.employee_name_by_idr;
		console.log('ABCD.employees ', employees);
		$.each($('.rider_selection'), function(index , value){	
		  for (var employeer in employees) {		
              		$(this).append($("<option>" + employees[employeer].name + "</option>").attr("value", employees[employeer].name).attr("id", employees[employeer].id))
                  }
		});
		console.log(self.get_orders());
...
//            if(result && result.lines.length > 0){
            	var selectedOrder = this.pos.get_order();
		console.log("pos_get_order: ",result);
            	self.pos.chrome.screens.orderlist.clear_cart();
            	selectedOrder.set_client(null);
            	if (result.partner_id && result.partner_id[0]) {
                    var partner = self.pos.db.get_partner_by_id(result.partner_id[0]);
				...
				console.log("pos_reference: ",result.pos_reference);
           	 	selectedOrder.set_pos_reference(result.pos_reference);
           	 	selectedOrder.set_order_id(order_id);
				console.log("name (sequence) : ",result.name);
           	 	selectedOrder.set_sequence(result.name);
//	           	if(result.lines.length > 0){
	           		if(result.table_ids && result.table_ids.length > 0){
	           			if(result.rest_table_reservation_id.length > 0){
